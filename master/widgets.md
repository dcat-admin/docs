...
`Dcat\Admin\Widgets\Box`用来生成box组件：
...
`Dcat\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Dcat\Admin\Widgets\Form`类用来快速构建表单：
...
`Dcat\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
效果请参考后台首页的布局文件[HomeController.php](https://github.com/z-song/dcat-admin/blob/master/src/Console/stubs/HomeController.stub)的`index()`方法中，关于`InfoBox`的部分。

## Tab组件

`Dcat\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`Dcat\Admin\Widgets\Table`类用来生成表格：
